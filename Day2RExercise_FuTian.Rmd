...
```{r}
count <- 1:nrow(gapminder)

for (i in count) {
  if (gapminder[i, "continent"]=="Americas"){
    if (gapminder[i, "country"]=="Canada" |
        gapminder[i, "country"]=="Mexico" |
        gapminder[i, "country"]=="United States") {
      gapminder[i, "newContinent"] <- "NorthAmerica"
    } else {
      gapminder[i, "newContinent"] <- "SouthAmerica"
    }
  } else{
    gapminder[i, "newContinent"] <- gapminder[i, "continent"]
  }
}

for (i in unique(gapminder$newContinent)) {
  plot(gapminder$log_lifeExp[gapminder$newContinent==i] 
       ~ gapminder$log_gdpPercap[gapminder$newContinent==i],
         main=i,
         xlab = "gdp per capita", ylab = "life expectancy")
}
